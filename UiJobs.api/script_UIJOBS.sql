...
CREATE PROCEDURE PorcAderencia
		@idVagas INT
AS
	BEGIN
		SELECT CO.nomeConhecimento AS [Nome Conhecimento], 
			   N.niveisDisponiveis AS [Nivel Conhecimento], 
			   CC.idCurriculo,
			   CASE
					WHEN CC.idNivel > VC.idNivel THEN 2  --Checar 
					WHEN CC.idNivel = VC.idNivel THEN 1 
					ELSE                              0 
			   END Aderencia 
		INTO #TEMP_CANDIDATOS
		FROM VagasConhecimentos VC
		INNER JOIN  Conhecimentos CO ON CO.idConhecimentos = VC	.idConhecimentos
		INNER JOIN	Nivel N ON N.idNivel = VC.idNivel AND N.idNivel = VC.idNivel
		INNER JOIN  CurriculoConhecimentos CC ON CC.idConhecimentos = VC.idConhecimentos
		WHERE VC.idVagas = @idVagas --AND CC.idCurriculo IS NOT NULL

		select * from #TEMP_CANDIDATOS

		DECLARE @Conhecimentos FLOAT
		SELECT @Conhecimentos = COUNT(*) FROM VagasConhecimentos VC WHERE VC.idVagas = @idVagas
		...
		select idCurriculo [Número Curriculo], Aderencia [Porcentagem Aderencia] from #TEMP_Aderencia
		ORDER BY Aderencia DESC

		UPDATE VC
		SET percAderencia = TA.Aderencia
		FROM #TEMP_Aderencia TA
		INNER JOIN VagasCandidato VC ON VC.idCurriculo = TA.idCurriculo
